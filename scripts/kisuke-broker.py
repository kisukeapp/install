...
class KisukeBrokerSDK:
    ...
    async def _handle_ios_client(self, ws: WebSocketServerProtocol):
        ...
        try:
            async for raw in ws:
                await self._dispatch_ios_msg(s, json.loads(raw))
    ...
    async def _start_claude(self, s: Session, cfg: Dict[str, Any]):
        ...
        try:
            ...
            log.debug("Connecting to Claude SDK...")
            log.debug("PATH environment: %s", os.environ.get('PATH', 'Not set'))
            log.debug("Looking for claude executable...")
            
            # Search for Claude CLI in expected locations
            claude_paths = [
            ...
            for claude_path in claude_paths:
                if os.path.exists(claude_path) or os.system(f"which {claude_path} >/dev/null 2>&1") == 0:
                    log.debug("Found claude at: %s", claude_path)
                    claude_found = True
                    break
            
            if not claude_found:
                log.warning("Claude CLI not found in any expected location")
...
async def _main():
    ...
    for sig in (signal.SIGTERM, signal.SIGINT):
        loop.add_signal_handler(sig, lambda: asyncio.create_task(broker.start()))
    await broker.start()
