...
CHECK_INTERVAL = 2  # Interval between port checks in seconds
...
class TmuxMonitor:
    ...
    def __init__(self):
        ...
        # Send debug confirmation of initialization
        self.emit_event({
            'type': 'DEBUG_STARTUP',
            'message': f'Forwarder initialized for session: {TMUX_SESSION}'
        })
    ...
    def capture_pane_output(self, pane_id):
        ...
        Returns:
            String containing last 100 lines of pane output, or empty string on error.
        """
        try:
            result = subprocess.run(
    ...
    def extract_ports(self, text):
        ...
        # Log sample of text being searched for debugging
        if text.strip():
            debug_text = text[:200].replace('\n', '\\n').replace('\r', '\\r')
            self.emit_event({
                'type': 'DEBUG_SCAN',
                'text_sample': debug_text,
            ...
            for match in matches:
                try:
                    ...
                    if 1024 <= port <= 65535:  # Check valid port range
                        ports[port] = (protocol, "/")
                        # Emit debug event for detected port
                        self.emit_event({
                            'type': 'DEBUG_PORT_FOUND',
                            'port': port,
                            'protocol': protocol,
    ...
    def monitor_panes(self):
        ...
        while self.running:
            try:
                ...
                # Log number of panes found
                self.emit_event({
                    'type': 'DEBUG_PANES_FOUND',
                    'count': len(panes),
                    'panes': panes
                })
                ...
                for pane_id in panes:
                    ...
                    detected_ports_info = self.extract_ports(output)
                    
                    # Log pane scan results
                    if output and len(output) > 0:
                        self.emit_event({
                            'type': 'DEBUG_PANE_SCAN',
                            'pane_id': pane_id,
                            'text_length': len(output),
                            'ports_found': len(detected_ports_info)
                        })
                    ...
                    for port, (protocol, path) in detected_ports_info.items():
                        if port not in self.detected_ports:
                            ...
                            # Log port availability check result
                            self.emit_event({
                                'type': 'DEBUG_PORT_CHECK',
                                'port': port,
                                'active': port_active
                            })
                ...
                for port in list(self.detected_ports.keys()):
                    ...
                    if port_active and not port_info.get('was_active', False):
                        ...
                        self.emit_event({
                            'type': 'DEBUG_PORT_BECAME_ACTIVE',
                            'port': port,
                            'age_seconds': int(port_age)
                        })
                    elif port_active:
                        port_info['was_active'] = True
